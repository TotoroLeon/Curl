...
class CurlCli extends \Phalcon\Mvc\User\Component{
    ...
    public static function factory($config = NULL)
    {
        ...
        {
            self::$instance = new CurlCli($config);
        }
    ...
    private $options = array(
            ...
            CURLOPT_ENCODING => '',
            CURLOPT_USERAGENT => 'Mozilla/5.0 (Windows NT 5.1; rv:22.0) Gecko/20100101 Firefox/22.0', //'CurlCli API Utility/1.0 (compatible;)',
            CURLOPT_DNS_CACHE_TIMEOUT => 1800
    );

    private $reqest_headers = array();
    ...
    {
        $this->reqest_headers = array('Expect:');
    ...
    public function post($url, $data = array(), $referrer = NULL, $timeout = 5, $headers = array())
    {
        ...
        $this->options[CURLOPT_POSTFIELDS] = $this->getPostData($data);

        $ret = $this->execute(FALSE);
        return $ret === FALSE ? FALSE : $this->response();
    }

    public function getPostData($params = array())
    {
        $isFileUpload = FALSE;
                ...
                if(preg_match('/^@(.+)$/', $param, $matches))
                {
                    if (is_file($matches[1]))
                    {
                        ...
                        else 
                      {
                          ...
                          $mimeType = finfo_file($finfo, $matches[1]);
                          finfo_close($finfo);
                          
                          $params[$k] = curl_file_create($matches[1], $mimeType, $pathInfo['basename']);
                        }
                    }
                }
            }
        ...
        return $isFileUpload ? $params : http_build_query($params);
    }
    ...
    public function response()
    {
        ...
        $this->destroy_curl();
        $results = explode("\r\n\r\n", $this->response,  2 + $response_info['redirect_count']);
        if (IS_DEBUG_CURL AND in_array($response_info['http_code'], array(500, 501, 502, 503, 504)))
        {
            print_r($response_info);
            print_r($this->response);
            $msg = $response_info['http_code'].':'.$this->response_body;
        ...
        $total = count($results);
        $this->response_header = $results[$total-2];
        $this->response_body = $results[$total-1];
